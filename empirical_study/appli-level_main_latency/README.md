# Application-Level Latency Analysis

This experiment demonstrates that application-level latency dominates cold start overhead.

## Measurement Metrics

Four key metrics are collected:

### Package Loading Analysis
- **Cold start latency**: 743.199324ms (measured via instrumentation)
- **Warm start latency**: 434.108498ms

Simulate cold start with:
```bash
find /home/app -type d -name "__pycache__" -exec rm -rf {} +
```

### Container Initialization Breakdown
Monitor via: `sudo journalctl -u containerd -f`

- **Create Pod Sandbox**: 0.527s (network namespace setup)
- **Pull Image**: 0.344s (from registry)
- **Create Container**: 0.336s (filesystem creation)
- **Start Container**: 0.191s (Python interpreter launch)

**Total container init**: 863ms (527 + 336)

**Analysis**: Application layer contributes 41% of total latency
`=(434+309)/(434+309+191+863)`
