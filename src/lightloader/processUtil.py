...
def getDynamicContent_new(seedfun_list, path, jsoninput, handler_file, used_func_path):
    ...
    for handler_i in handler_file:
        use_func = handler_i_hanler(handler_i, use_func,load_dict) # handler_file_handle is above
    ...
    with open(used_func_path, "w") as w:
...
def getDynamicContent(seedfun_list, path, jsoninput, handler_file, moshu_file, dynamic_file_result_output):
    ...
    for handler_i in handler_file:
        use_func = handler_i_hanler(handler_i, use_func,load_dict) # for single .py file
...
def extend_func(main_func, load_dict):
    ...
    """
    # changes into pycg output format
    used_func_list = []
    for key,value in load_dict.items():
        if isinstance(value,list):
            used_func_list.extend(value)
    used_func_list = [num for num in set(used_func_list) if not num.startswith('<builtin>')]
    return used_func_list
...
def seed_func_relation(main_func, load_dict, rel_dir):
    ...
    for use_func_i in use_func: # add functions that don't belong to built-in functions
        if '<builtin>' not in use_func_i:
    ...
    for key_i in use_functions:
        ...
        flag, target_file, loc = findfile(rel_dir, temp_key, 0)# determine the type, package, .py
            
        if flag ==1: # just a .py file
...
def result_addlibray(dynamic_file_result_output_re, add_libray, prefunc_dir, used_fun_final):
    # add import part for rewriting
    save_result = ['custom_funtemplate.rewrite_template']  # write to a new package
