...
def get_ast(input_file):
    ...
    global ast_dict
    if input_file not in ast_dict:
        with open(input_file,'r',encoding='utf-8') as f:
            content = f.read()
        tree_node = ast.parse(content)
        ast_dict[input_file] = tree_node
        return tree_node
    else:
        return ast_dict.get(input_file)
...
def update_key(load_dict):
    ...
    add_key = []
    for value in load_dict.values(): # value is a list
        for i in value:
            if not i in keylist:
                add_key.append(i)
...
def  update_output(load_dict, rel_dir, dir_name, py_all):
    ...
    for key_i in load_dict.keys():
        ...
        flag, target_file, loc = file_exsit(rel_dir, temp_key, 0) # return:  3-> package(contains a __init__.py)  1-> contains .py file,  0->not package nor file   2->package but .py not exist
        find_file = ''
        fun_component = ''
...
def add_info(path, jsoninput, handler_file, moshu_file, jsonoutput):
    ...
    py_all = get_all_value(path) # all __ALL__ or __all__ var/def
    # print('all the pyall is ',py_all)
    dir_name = "{}/".format(path)
    # map, key is empty
    load_dict = read_context(jsoninput) # handler function info
    ...
    if not os.path.exists(jsonoutput):
        with open(jsonoutput, 'w') as file:
            pass
...
# match handler with corresponding variables and classes,
def handler_file_handle(handler_i, load_dict, dir_name, py_all, moshu_file):
    ...
    rel_dir = ".".join([path for path in rel_dir]) # change into package import form, and not include final file
    
    # py_all key is to unify with dict as . connected form
    load_dict = update_output(load_dict, rel_dir, dir_name, py_all) # 
    # print ( 'afer the first update,  the dict is ',load_dict,'the rel_dir is ',rel_dir,'the dir_name is ',dir_name)
    add_key = update_key(load_dict) # input a map, key is handler function .path. return value is a list, combine all value content together, deduplicate
    # lxml.html has no content
    special_key = ['numpy.fromfunction']
    # print( f"before add_key is {len(add_key)}")
    add_key.extend(special_key)
    # pick from moshu_file to add_key
    for line in open(moshu_file):
        line = line.strip('\n')
        add_key.append(line)
    # add_key = {package from handler} + {special magic function} 
    # here continuously recurse to parent directory, will be very slow

    while len(add_key)>0:
        # print(f'add-key len is {len(add_key)}')
        add_keydict = {}
    ...
    # here load_dict key/value added a lot of content, currently not understandable
    # print( f"after2 add_key is {len(add_key)}")
    return load_dict
    ...
    for root, dirs, files in os.walk(path):
        for name in files:
            if name.endswith('.py'):
                ...
                all_tmp = get_assign(tree_node)   # public API for module, will be imported when using `from module import *`
